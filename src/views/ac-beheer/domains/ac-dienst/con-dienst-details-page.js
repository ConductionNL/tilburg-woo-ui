...
import ConDienstDetailsPageContent from './con-dienst-details-page-content';

/**
 * Dienst Details Page (fixed type 'dienst')
 * Mirrors product/module/contactpersoon pages using DetailsPageConfigFactory
 */
const ConDienstDetailsPage = ({ store }) => {
  const { object, user } = store;
  const navigate = useNavigate();
  const params = useParams();
  const id = params?.id;

  const pageType = 'dienst';
...
export default withStore(observer(ConDienstDetailsPage));
