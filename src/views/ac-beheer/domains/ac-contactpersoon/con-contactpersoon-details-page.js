...
import ConContactpersoonDetailsPageContent from './con-contactpersoon-details-page-content';

/**
 * Contactpersoon Details Page (simplified for fixed type)
 * - Fixed config for contactpersonen; no dynamic type switching
...
const ConProductDetailsPage = ({ store }) => {
  ...
  // Fixed page type and configuration (contactpersonen)
  const pageType = 'contactpersonen';
      ...
      <AcFlex spacing='xl'>
        ...
        <div className='ac-beheer-details--100-width'>
          <AcColumn gap='sm'>
            ...
            {!loading && data && (
              <ConContactpersoonDetailsPageContent
                loading={loading}
                config={config}
                data={data}
                userStore={user}
