...
const AcFormsProduct = ({ userStore, store }) => {
  ...
  const getExistingApplications = useCallback(() => {
    return getExistingModulesWithLookupData().map(module => module.naam || 'Unnamed Module');
  ...
  const loadReferentieComponenten = useCallback(async () => {
    ...
    try {
      ...
      console.log('🔍 Full endpoint:', endpoint);
        ...
        // Use object store's fetchCollection method which includes authentication
        await store.object.fetchCollection('vng-gemma', 'element', queryParams);
        const data = store.object.getCollection('vng-gemma_element');
        const list = Array.isArray(data?.results) ? data.results : [];
  ...
  const performModulesSearch = useCallback(async (searchTerm = '') => {
    ...
    try {
      ...
      // Use object store's fetchCollection method which includes authentication
      await store.object.fetchCollection('voorzieningen', 'module', queryParams);
      const data = store.object.getCollection('voorzieningen_module');
      const list = Array.isArray(data?.results) ? data.results : [];
      ...
      case 6:
        return (
          <ConFormReferentiecomponentenStage
            ...
            setReferentieComponentenWithStandards={setReferentieComponentenWithStandards}
            schemas={schemas}
  ...
  const getDisabledStatus = (currentStep) => {
    ...
    if (logicalStep === 3) {
      ...
      const hasIncompleteNewModules = newModules.some(module => {
        return !module.naam || !module.naam.trim() || !module.beschrijvingKort || !module.beschrijvingKort.trim();
      });
    ...
    if (logicalStep === 3) {
      ...
      newModules.forEach((module, index) => {
        ...
        if (missingFields.length > 0) {
          ...
          incompleteModules.push(`${moduleName}: ${missingFields.join(', ')} ontbreekt`);
        }
      });
