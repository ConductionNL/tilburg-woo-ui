...
const StandaardenFormNew = ({
  ...
  // Filter out existing applications - they don't get standards configuration
  const newApplications = useMemo(() => {
    return Object.entries(product.applicaties || {})
      .filter(([id, app]) => !app.isExisting)
      .map(([id, app]) => ({ id, ...app }));
  }, [product.applicaties]);
  ...
  console.log('🔍 StandaardenFormNew Debug:', {
    newApplications: newApplications.map(app => ({ id: app.id, naam: app.naam })),
  ...
  useEffect(() => {
    ...
    newApplications.forEach(app => {
      console.log('🔍 Processing app:', { appId: app.id, appNaam: app.naam });
      
      allStandards.forEach(standard => {
        // Only include standards that are relevant to this module
        if (String(standard.moduleId) === String(app.id)) {
          const key = `${app.id}-${standard.id}`;
          
          // Check if there's existing compliancy data
          const existingCompliancy = app.compliancy?.find(c => c.standaardversie === standard.id);
          ...
          initialState[key] = {
            moduleId: app.id,
            moduleName: app.naam || `Applicatie ${app.id}`,
  ...
  }, [newApplications, referentieComponentenWithStandards]);
  ...
  const toggleCompliance = (key, isCompliant) => {
    ...
    setProduct(prev => {
      ...
      const app = applicaties[entry.moduleId] || {};
  ...
  // If no new applications exist, show a message
