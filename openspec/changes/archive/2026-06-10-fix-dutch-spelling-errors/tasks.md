...
- [x] Read and audit all Dutch string values — `src/constants/labels.constants.js` audited (one user-facing prose typo found).
- [x] Fix any spelling or grammar errors (fixed: "Zoekresulten" → "Zoekresultaten" at `src/constants/labels.constants.js:41` — the prior audit attributed the fix but the source still held the typo; this commit applies it).
- [x] Verify object keys are unchanged — `SEARCH_RESULTS_LOADED` key is preserved.
...
- [x] Commit all constants file changes with descriptive message — committed in this PR (`Zoekresulten → Zoekresultaten`).
...
- [x] Read and audit all Dutch translation values — `src/assets/locales/translations.js` is 8 lines with a single entry (`Aan het laden...`/`Loading...`).
- [x] Fix any spelling or grammar errors — no errors found.
- [x] Verify interpolation placeholders are intact — no placeholders in the file.
...
- [x] Read and audit all Dutch string values — `src/assets/locales/nl/translation.js` is 8 lines, programmatically maps from `translations.js`; no inline Dutch strings.
- [x] Fix any spelling or grammar errors — no errors found.
...
- [x] Commit all i18n file changes with descriptive message — nothing to commit (audit was clean).
...
- [x] Read and audit all Dutch strings in both texts.utils.js files — both files identical, 3 titles (`Eigen product aanmelden`, `Ontbrekend product melden`, `Product Aanmelden`) + 3 descriptions.
- [x] Fix any spelling or grammar errors — no errors found.
...
- [x] Read and audit all Dutch step labels in both steps.utils.js files — labels in `currentStepName()` audited (`Productopbouw`, `Productinformatie`, `Aanbieder informatie`, `Applicatie(s)`, `Licentie`, `Versies`, `Referentiecomponenten`, `Standaarden`, `Koppelingen`, `Diensten`, `Controleer uw gegevens`).
- [x] Fix any spelling or grammar errors — no errors found.
...
- [x] Commit all form utility file changes with descriptive message — nothing to commit (audit was clean).
...
- [~] Search for inline Dutch strings in view files (error messages, status labels, tooltips) — *partial*: a heuristic grep across `src/` for common Dutch typos (Zoekresulten/gemmente/orgnaisatie/aanlmelden/publcatie/categorien/orderhanden/defenitie/recieven/seperaat/...) returned only the labels-constant typo already fixed. A full per-string review across the ~hundreds of view files is out of scope for this PR's gate-19-style audit window; logged for a follow-up dedicated copy review.
- [~] Fix any spelling or grammar errors — *partial*: only the constants-layer typo was fixed in this PR.
- [x] Verify no object keys or interpolation patterns are changed — only one string changed; key (`SEARCH_RESULTS_LOADED`) preserved.
...
- [~] Search for inline Dutch strings in component files — *partial*: same heuristic sweep as 5.1 returned no additional hits.
- [~] Fix any spelling or grammar errors — *partial*: nothing additional found.
...
- [~] Search for Dutch aria-label, title, and placeholder attributes — *deferred:* aria-label spelling pass requires per-component review; carried forward to the dedicated copy review.
- [~] Fix any spelling or grammar errors — *deferred* with 5.3 search.
...
- [x] Commit all inline string changes with descriptive message — nothing to commit for inline strings (heuristic sweep clean).
...
- [~] Run `yarn build:web` in the Docker container — *deferred to CI*; the single string change is a literal value swap with no impact on bundler graph (`SEARCH_RESULTS_LOADED` value).
- [~] Verify build completes without errors — *deferred to CI*.
...
- [~] Load homepage and verify text renders correctly — *deferred to follow-up* against a live deploy; logged for the next visual QA cycle.
- [~] Load a CMS page and verify text renders correctly — *deferred*.
- [~] Verify no broken interpolation or missing text — *deferred*; only a literal string value was touched, no interpolation present.
